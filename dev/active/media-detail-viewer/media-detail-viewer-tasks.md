...
- [x] **2-6** 편집 기능 범위 확정
  ...
  - 메모: Issue #6에서는 현재처럼 안내 Alert를 유지하고, 실제 사진 편집/crop-only 구현은 GitHub Issue `#12`로 분리함
...
- [ ] **3-11** 상세정보 시트와 사진 동반 이동 구조 정리
  ...
  - 메모: 최신 코드에서는 `MediaDetailRevealGeometry.pageContentLiftOffset`를 추가하고, `TabView` 자체가 아니라 각 `MediaPageView` 렌더 표면에 `visualEffect` 기반 lift를 적용함. 사용자 확인 기준 reveal 중 사진 떨림은 사라졌지만, dismiss 마지막에 기본 배율 사진이 중앙으로 jump 하듯 움직이는 후속 문제가 남아 있어 완료 처리하지 않음
...
- [ ] **5-5-q-1** Photos 스타일 시트 표현에서 메타데이터 표시 검증
  - 메모: 패널에는 캐시된 full details만 표시하고, 내부 날짜/메타데이터 행의 위치와 높이를 고정하며 긴 텍스트는 말줄임 처리하도록 보정함. 추가로 현재 사진의 full details가 준비된 뒤 패널을 열도록 바꿔 reveal 중 `loading -> content` 전환이 끼어들지 않게 함. 사진 떨림 완화를 위해 기본 배율에서는 내부 `UIScrollView` pan/bounce를 막고, image centering frame 조정은 무애니메이션으로 처리함. 미세한 bounds 차이로 zoom reset이 반복되지 않도록 컨테이너 크기 비교를 정규화함. 최신 수정에서 사진 lift는 page content `visualEffect`로 복원했으며, 실제 reveal 체감 확인이 남아 있음
...
- [x] **5-5-r** 편집 액션 정책 검증
  - 메모: 미구현 정책을 확정함. Issue #6에서는 안내 Alert를 유지하고, crop-only 저장/취소/원본 보존 정책 검증은 Issue `#12`로 넘김
...
- [x] **5-6** iPad 레이아웃/회전에서 기본 동작 이상 없는지 확인
...
- [x] 사진 편집 기능 후속 이슈 작성
  - 메모: GitHub Issue `#12` `미디어 상세 뷰 사진 편집 기능 구현`
